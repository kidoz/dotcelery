...
{
    public static DiagnosticDescriptor Diagnostic(DiagnosticDescriptor descriptor) => descriptor;

    public static async Task VerifyAnalyzerAsync(
        string source,
        params DiagnosticDescriptor[] expectedDescriptors
    )
    {
        ...
        var unexpectedDiagnostics = diagnostics
            .Where(d => !expectedDescriptors.Any(ed => ed.Id == d.Id))
            .ToList();
        if (unexpectedDiagnostics.Any())
        {
            var messages = string.Join(
                Environment.NewLine,
                unexpectedDiagnostics.Select(d => $"  - {d.Id}: {d.GetMessage()}")
            );
            throw new InvalidOperationException(
                $"Unexpected diagnostics found:{Environment.NewLine}{messages}"
            );
        }
    }
    ...
    private static CSharpCompilation CreateCompilation(string source)
    {
        ...
        var references = new List<MetadataReference>
        {
            ...
            MetadataReference.CreateFromFile(
                typeof(DotCelery.Core.Abstractions.ITask).Assembly.Location
            ),
        };
    ...
    private static (string code, List<TextSpan> locations) ExtractMarkedSpans(string markedSource)
    {
        ...
        for (var i = matches.Count - 1; i >= 0; i--)
        {
            ...
            var adjustedStart =
                start - (matches.Count - 1 - i) * (match.Value.Length - content.Length);
